...
use fugue_causal::{
    infer_causal, prior_ate, DoublyRobust, PluginEstimator, Estimand,
};
...
fn main() {
    ...
    let observations: Vec<(Vec<f64>, f64, f64)> = data
        .iter()
        .map(|(x, a, y)| (vec![*x], *a, *y))
        .collect();
    ...
    match infer_causal(
        ...
        Ok(posterior) => {
            ...
            println!("  95% Credible Interval: [{:.4}, {:.4}]", ci_lower, ci_upper);

            // Check coverage
            let covers_truth = ci_lower <= true_ate && true_ate <= ci_upper;
            println!("  Contains Truth:    {}", if covers_truth { "✓" } else { "✗" });
            ...
            println!("\n  Bias:    {:.4}", bias);
            println!("  RMSE:    {:.4}", rmse);
            println!("  Coverage: {}", if covers_truth { "valid" } else { "invalid" });
        }
